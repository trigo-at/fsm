...
class FSM {
	...
	execute(trasition, ...args) {
		return this[FSM.toFunctionName(trasition)](...args);
	}
	...
	addTransition(transition) {
		...
		transitions.map(parseTrasition).forEach((transArray) => {
			transArray.forEach((trans, index) => {
				if (index === 0 && this._trasitionFunctionNames.indexOf(FSM.toFunctionName(trans.name)) !== -1) {
					const conflicting = this._trasitionFunctionNames[this._trasitionFunctionNames.indexOf(FSM.toFunctionName(trans.name))];
					throw new Error(`Abigious transtion name: "${trans.name}" conflicts with existing transition: "${conflicting}"`);
				}
				if (index === 0) {
